...
makeCacheMatrix <- function(x = matrix()) {
        ...
        set <- function(y) { ## creates the matrix in the working environment
                x <<- y
                inv_x <<- NULL ## The use of '<<-' is to assign a value to an object in an environment
...
cacheSolve <- function(x, ...) {
        ...
        if (!is.null(inv_x)) { ## returns the inverted matrix from the cache if it exists
                message("Please hold...getting cached inverse matrix") ## Displays the message
                return(inv_x) ## Displays the Matrix in console
        } else {
                inv_x <- solve(x$get()) ## A matrix will be created since it does not exist
                x$setinverse(inv_x) ## sets the inverted matrix in cache
